...
public class JMemoFrame12 extends JFrame {
	// 아이콘
	public static final ImageIcon IC_INFO 
		= new ImageIcon("./icons/information.png"); 
...
//				if(  rdColor.getText().equals("검정")) {}
	...
	JMemoFrame12 frm; // 프레임창의 자신의 주소를 기억..

	public static final String DEF_PATH = "./mymemo/";
	public static final String DEF_EXT = ".my"; // 확장자
	String filePath; // 저장/읽어오기 파일경로
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					JMemoFrame12 frame = new JMemoFrame12();
	...
	public JMemoFrame12() {
		...
		btnLoad.setIcon(new ImageIcon("C:\\dev2021\\java_ws\\GUIMemoProject\\icons\\book_open.png"));
		btnLoad.setToolTipText("\uBA54\uBAA8 \uC77D\uC5B4\uC624\uAE30");
		panel.add(btnLoad);
		
		JButton btnSave = new JButton("");
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String memo = frm.textArea.getText();
				if( memo.isEmpty() ) return;
				boolean r = FileUtility
					.saveMemoToFile(memo, frm.filePath);
				if( r ) System.out.println(filePath + " 파일 저장 성공!");
				else System.out.println(filePath + " 파일 저장 실패!");
			}
		});
		btnSave.setIcon(new ImageIcon("C:\\dev2021\\java_ws\\GUIMemoProject\\icons\\script_save.png"));
		btnSave.setToolTipText("\uBA54\uBAA8 \uC800\uC7A5\uD558\uAE30");
		panel.add(btnSave);
		...
		// 익명객체로 구현한 객체를 인터페이스 다형성으로 사용..
		btnAbout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
//				this. // this는 익명객체 자신.
				// owner는 대화상자를 띄워주는 부모창의 주소
				// 1.
//				JDialog dlg = 
//					//new JDialog(frm, "About My메모장..", true);
//					new JDialog(frm, "About My메모장..", false);
//					// true면 모달 모드, false 모달리스 모드
//				dlg.setBounds(300, 300, 240, 120);
//				JLabel lbInfo = new JLabel(
//					"우리 메모장은 최근 디자인으로...",
//					IC_INFO, JLabel.CENTER);
//				dlg.getContentPane().add(lbInfo);
//				dlg.setVisible(true);
				// 2. 커스텀 대화상자 띄우기
				JInfoDialog infoDlg = new JInfoDialog(frm);
				infoDlg.setVisible(true);
				
			}
		});
		btnAbout.setIcon(new ImageIcon("C:\\dev2021\\java_ws\\GUIMemoProject\\icons\\heart.png"));
		btnAbout.setToolTipText("My\uBA54\uBAA8\uC7A5 \uB300\uD558\uC5EC...");
		panel.add(btnAbout);
...
//			private void applyNewFontsizeToTextarea(int newSize) {
...
//		});
	...
	// 폰트 사이즈 && 폰트이름 설정 함수
	protected void applyNewFontsizeToTextarea(
			String fontName, int newSize) {
	...
	protected void applyNewFontsizeToTextarea(int newSize) {
		// 기존 폰트의 속성을 복제한 새폰트를 생성하여 textarea에 적용..
		Font oldFont = this.textArea.getFont();
		Font newFont = new Font(oldFont.getFontName(),
				oldFont.getStyle(), newSize);
		textArea.setFont(newFont);
	}
