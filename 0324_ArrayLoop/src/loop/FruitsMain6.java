...
public class FruitsMain6 {
	...
	public static void main(String[] args) {
		...
		int lang = ScanUtill.userInputToIntInRange(
			...
			String msg = (lang == LANG_KR) ?  
					"사용자 이름을 입력하세요: "  : 
					"Insert user's name in English: ";
			String userName =					
					ScanUtill.userInputToString(msg);
			
			int len = (lang == LANG_EN) ? fruitNamesEn.length : 
						fruitNamesKr.length;
			int frName = (int)(Math.random()*len); // 10 or 13
			
			userEatsFruit(userName, (lang == LANG_KR)  ? 
				fruitNamesKr[frName] : fruitNamesEn[frName] );
