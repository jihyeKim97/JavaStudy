...
// about 버튼 전용 이벤트 처리 클래스 정의
class AboutButtonHandler implements ActionListener {
	JMemoFrame7 frm;
	public AboutButtonHandler(JMemoFrame7 frm) {
...
//			new JDialog(frm, "info 대화상자..", true); // 모달 modal
			new JDialog(frm, "info 대화상자..", false); // 모달리스 modaless
...
//	private JTextField txtMemo; // 한 줄 짜리 문자열 입력
	...
	public JMemoFrame7(String title) {
		// 프레임 창 설정....
		super(title);
...
//		this.add(lbStatus, BorderLayout.SOUTH);//상태바
	...
	public static void main(String[] args) {
		JMemoFrame7 frm 
			= new JMemoFrame7("나의 메모장2");	
		//frm.setVisible(true);
	}

	// 이벤트 처리 콜백함수 (callback): 개발자가 직접 호출하지 않고,
	// 특정 메커니즘이나 정책 혹은 시스템의 규칙에 의해 호출되는 함수
	@Override
	public void actionPerformed(ActionEvent e) {
		// ActionEvent e 콜백이 발동될 때의 이벤트문맥 정보 객체
		JButton evBtn = (JButton) e.getSource();
			// evBtn 이벤트가 발생한 컴포넌트의 주소		
		if( this.btnSave == evBtn ) {
			System.out.println("메모 저장 버튼 눌렸음!!");
		} else if( this.btnLoad == evBtn ) {
			System.out.println("메모 읽기 버튼 눌렸음!!");
		} else if( this.btnChangeColor == evBtn ) {			
	...
	//		bColorBlackOnWhite = !bColorBlackOnWhite;
			...
			System.out.println("아직 처리할 수 없는 기능 버튼!");
		}
	}
