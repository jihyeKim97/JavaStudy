...
public class JSwingFrame6 extends JFrame implements ActionListener {
...
//		super("무명 프레임");
	...
	public JSwingFrame6(String title) {
		super(title);
...
//			new GridLayout(2, 2, 20, 10)); // 2 x 2
		...
		String opIconNames[] = { "add", "delete", "cross", "cut" }; // 더하기 add.png 파일, 빼기, 곱하기, 나누기(cut?? divide?)
																	// "./icons/add.png"
																	// "./icons/" + "add" + ".png"
		this.rdBtns = new JRadioButton[rdLen];
		final ImageIcon selV = new ImageIcon("./icons/tick.png");
		// v체크 => 선택된 상태
//		JPanel pn3 = new JPanel(); // flowlayout		
		JPanel pn3 = new JPanel(new GridLayout(4, 1));

		ButtonGroup rdGroup = new ButtonGroup();
		// 라디오버튼들을 동일 그룹으로 묶어서 하나의 UI처럼 동작하게 만듬.
		// 즉, 배타성을 통해서 오직 한 순간에 그룹에서는 하나의 라디오버튼만 true로 선택됨.

		for (int j = 0; j < opIconNames.length; j++) {
			ImageIcon ic = new ImageIcon("./icons/" + opIconNames[j] + ".png");
			JRadioButton rd = new JRadioButton(opIconNames[j], ic);
	...
	public static void main(String[] args) {
		JSwingFrame6 frm = new JSwingFrame6();
//		frm.setVisible(true);
	}
