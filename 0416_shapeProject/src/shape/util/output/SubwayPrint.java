...
public class SubwayPrint {
	...
	public static void printMyTriangle(MyTriangle tr) {
		...
		System.out.println(dummy);
		//System.out.println(TAIL);
		System.out.printf(TAIL2, 
				tr.tag == null || tr.tag.isEmpty()
				? "삼각형" : tr.tag
			);
	}	
...
//		"    ctx.fill();";
	...
	public static void 
		printMyTriangleWithStyle(
				MyTriangle tr, boolean black) {
		MyPoint ptA = tr.getPtA();
		MyPoint ptB = tr.getPtB();
		MyPoint ptC = tr.getPtC();
		String lnColor = black ? 
				MyShape.COLOR_BLACK: tr.color; // tr.getLnAB().color;
		//String dummy = "ctx.beginPath();\r\n" +
		//String dummy = "ctx.strokeStyle = '#000000';";
		String dummy = "ctx.strokeStyle = '#"+lnColor +"';\r\n" +
		"    ctx.lineWidth = "+tr.getLnAB().width +";\r\n" +
	...
	// 선객체 하나 받아서 html 출력
	public static void 
		printMyLine(MyLine ln, boolean black) {
		MyPoint ptS = ln.getStartPoint();
		MyPoint ptE = ln.getEp();
		
		String lnColor = black ? 
				MyShape.COLOR_BLACK: ln.color; 
		String dummy = "ctx.strokeStyle = '#"+lnColor +"';\r\n" +
		...
		dummy += 
			" ctx.fillText('Line', "
					+ptE.x+"+20, "+ptE.y+"+20);";
		String fmtDist = String.format("%.2f cm",
					ln.getDistance());
		dummy += 
			" ctx.fillText('     distance: "+ fmtDist +" ', "
				+ptE.x+"+20, "	+ptE.y+"+50);";		
	
		System.out.println(HEAD);
		System.out.println(dummy);
		System.out.printf(TAIL2, 
				ln.tag == null || ln.tag.isEmpty()
				? "선" : ln.tag
			);
	}	
	...
	// 원객체 하나 받아서 html 출력
	public static void 
		printMyCircle(MyCircle cc, boolean black, int width) {
		MyPoint ptC = cc.ptCenter;
		
		
		String lnColor = black ? 
				MyShape.COLOR_BLACK: cc.color; 
		String dummy = "ctx.strokeStyle = '#"+cc.color+"';\r\n" +
		"    ctx.lineWidth = "+width +";\r\n" +
		"    ctx.arc("+ptC.x +"," +ptC.y +","
			+ cc.radius + ", 0.0, Math.PI * 2, true);\r\n" + 
		"    ctx.stroke();";
		...
		dummy += 
			" ctx.fillText('Circle', "
					+ptC.x+"+20, "+ptC.y+"+20);";
		String fmtArea = String.format("%.2f ^m",
					cc.area);
		dummy += 
			" ctx.fillText('     Area: "+ fmtArea +" ', "
				+(ptC.x+cc.radius)+"+20, "
		...
		System.out.println(dummy);
		System.out.printf(TAIL2, 
				cc.tag == null || cc.tag.isEmpty()
				? "원" : cc.tag
			);
	}	
	
	
	// 서울메트로 지하철 노선도 객체를 받아서 출력하는 함수
	// 원객체 하나 받아서 html 출력
	public static void 
		printSubwayLine(SubwayLine sb, int width) {
		
		String dummy = ""; 
		
		// 13역 루프, 12 철로구간 루프
		//for (int i = 0; i < sb.sbStations.length; i++) {
		for (int i = 0; i < sb.sbNumberOfStations; i++) {
			// Station 출력
			//MyCircle cc = sb.sbStations[i]; // 상속다형성
			Station cc = sb.sbStations[i];
			MyPoint ptC = cc.ptCenter;			
			String lnColor = sb.sbColor;
			dummy += "ctx.beginPath();";
			dummy += "ctx.fillStyle = '#000000';\r\n";
			dummy += "ctx.strokeStyle = '#"+cc.color+"';\r\n" +			
			"    ctx.lineWidth = "+width +";\r\n" +
			"    ctx.arc("+ptC.x +"," +ptC.y +","
				+ cc.radius + ", 0.0, Math.PI * 2, true);\r\n" + 
			"    ctx.stroke();";
			...
			// RailWay 출력
			if( i > 0 ) {
				RailWay rw = sb.sbRailways[i-1];
				dummy += "ctx.beginPath();";			
				dummy += "ctx.strokeStyle = '#0000FF';\r\n" +
				"    ctx.lineWidth = "+ 2 +";\r\n" +
				"    ctx.moveTo("+rw.rwEs.ptCenter.x
							+", "+rw.rwEs.ptCenter.y+");\r\n" + 
				"    ctx.lineTo("+rw.rwSs.ptCenter.x
							+", "+rw.rwSs.ptCenter.y+");\r\n" +
				"    ctx.stroke();";			
				dummy += "ctx.closePath();";
			}
		...
		System.out.println(HEAD);
		System.out.println(dummy);
		System.out.printf(TAIL2,sb.sbName);		
	}	
...
//		printMyLine(ln, false);
		...
		printMyCircle(cc, false, MyShape.BOLD_WIDTH);
