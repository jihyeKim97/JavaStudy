 ...
 */
public class MyTriangle_backup4 {
	// 필드
	public static final String TYPE = "삼각형 TA";
	private MyPoint ptA; // rw 꼭지점 A ~ C
	private MyPoint ptB; // rw
	private MyPoint ptC; // rw
	private MyLine lnAB; // read only ptA-ptB 연결하는 선분
	private MyLine lnBC; // r ptB-ptC 연결하는 선분
	private MyLine lnCA; // r ptC-ptA 연결하는 선분
	public String tag;
	public String color;
	...
	// 세변의 길이 a, b, c라고 하고,
	// 둘레의 절반 hp = (a + b + c) / 2.0
	// 넓이 = sqrt( hp * (hp-a) * (hp-b) * (hp-c) )
	public double getArea() { // 측정치는 read-only
		double hp = this.perimeter / 2.0;
		double lnABDif = hp - this.lnAB.getDistance();
		double lnBCDif = hp - this.lnBC.getDistance();
		double lnCADif = hp - this.lnCA.getDistance();
		double area = Math.sqrt(hp * lnABDif * lnBCDif 
				* lnCADif );
		this.area = area;
		return area;
	}
	...
	// 삼각형의 바깥둘레
	// 세 변(선분)의 길이의 합
	public double getPerimeter() {// 측정치는 read-only
		return perimeter;
		//return this.perimeter;
	}
	public double measurePerimeter() {
		double perimeter = 
				this.lnAB.getDistance() +
				this.lnBC.getDistance() + 
				this.lnCA.getDistance();
		this.perimeter = perimeter;
		return this.perimeter;
	}
	
	public double getAngleCA_AB() {// 측정치는 read-only
		return angleCA_AB;
	}
	public double getAngleAB_BC() {// 측정치는 read-only
		return angleAB_BC;
	}
	public double getAngleBC_CA() {// 측정치는 read-only
		return angleBC_CA;
	}
	...
	// 배열의 길이가 3이상이 아니면.. 에러!!
	public MyTriangle_backup4(MyPoint[] pts) {
...
//				new MyLine(pts[2], pts[0], "삼각형 선분 CA"));
	...
	public MyTriangle_backup4(int ptAX, int ptAY, 
	...
	// (주어진 3개 점객들로 구성된 선분 객체 3개를 동적으로 new 생성)
	public MyTriangle_backup4(MyPoint ptA, MyPoint ptB, MyPoint ptC) {
		this(ptA, ptB, ptC, 
...
//		this.color = color;
	...
	// 풀 생성자 shift + alt + s
	public MyTriangle_backup4(MyPoint ptA, MyPoint ptB, MyPoint ptC, MyLine lnAB, MyLine lnBC, MyLine lnCA, String tag,
